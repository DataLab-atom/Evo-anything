...
**OpenClaw-Evo** 是论文 [arXiv:2503.10721](https://arxiv.org/abs/2503.10721) 的工程实现：
...
OpenClaw-Evo 是一个**纯粹的能力组合器**，自身不重造轮子，完全站在 OpenClaw 已有能力上。

### 依赖关系图

```
OpenClaw-Evo 用户指令（如 /hunt CIFAR-100-LT SOTA）
         │
         ▼
┌─────────────────────────────────────────────────────┐
│                  OpenClaw-Evo 插件层                  │
...
| OpenClaw-Evo 需要做的事 | 依赖 OpenClaw 哪个能力 |
|------------------------|----------------------|
...
OpenClaw-Evo 实现了论文提出的 **U2E 协议**：

| 维度 | EoH / FunSearch（前人工作） | OpenClaw-Evo（U2E） |
|------|--------------------------|-------------------|
| 模板依赖 | 需要预定义模板 | **无需模板** |
| 优化范围 | 局部单函数 | **全局多目标** |
...
OpenClaw 的 `MEMORY.md` 是通用对话记忆。OpenClaw-Evo 在此之上构建了**结构化的演化专用记忆**：
...
```
OpenClaw 已有能力                OpenClaw-Evo 升华为
─────────────────────────────────────────────────────
