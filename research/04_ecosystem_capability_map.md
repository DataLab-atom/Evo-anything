...
- 实时更新（文件变化自动刷新）
- JS 交互注入和截图能力
- **→ 可用于：实时展示 evolution 曲线、族谱图、per-target fitness 趋势**
...
- **→ 可用于：将 evolution loop 的各阶段定义为 lobster workflow，增强可观测性**
...
**→ Evo-anything 的 WorkerAgent 目前自己写代码变体，可考虑调用 coding-agent 做更复杂的重构**
...
#### `oracle` — 整仓库上下文代码分析
**Evo-anything 目前未使用，但 MapAgent 可大量受益**
...
**→ MapAgent 识别优化目标时，可用 oracle 做整仓库级别的深度分析，而不是逐文件 read**
...
**→ 可用于：回溯历次 evolution run 的决策过程和失败原因**
...
**→ 可用于：/hunt 阶段快速读取 arXiv 论文 PDF，提取方法摘要**
...
```
Layer 1 (平台内置):
  ✅ exec          → git 操作 / benchmark 执行 / pip install / 静态分析
...
Layer 2 (官方 Skills):
  ✅ github (exec "gh search repos") → /hunt 候选仓库搜索
...
| 现在做法 | 生态里已有的替代/增强 |
|---------|-------------------|
| `browser` 手动访问 paperswithcode.com | `arxiv-watcher` skill：直接搜索 arXiv 返回结构化论文列表 |
| 手动提取 repo 链接 | `academic-deep-research` skill：完整学术调研工作流 |
| `summarize` 手动读 README | `dizest-summarize`：自动提取 PDF 论文核心方法 |
| — | `pubmed-edirect`：生医领域论文搜索 |
...
| 现在做法 | 生态里已有的替代/增强 |
|---------|-------------------|
| 逐文件 `read` + `exec grep` | `oracle` skill：整仓库上下文一次性送给 LLM 分析 |
| 手动追踪调用链 | LSP integration skill：Language Server 函数跳转/引用图 |
...
| 现在做法 | 生态里已有的替代/增强 |
|---------|-------------------|
| 纯文字 /report | `canvas` tool：实时渲染 fitness 曲线、进化族谱图 |
| — | `grafana-lens`：将 evolution metrics 推送到 Grafana |
...
| 现在做法 | 生态里已有的替代/增强 |
|---------|-------------------|
| LLM 直接生成变体（单模型） | `coding-agent` skill：调用 Claude Code / Codex 等专业编程 agent |
| — | `agent-swarm` skill：多模型并行尝试，取最佳 |
| — | `lsp-integration` skill：变体生成前先做类型检查，减少低质量变体 |
...
| 现在做法 | 生态里已有的替代/增强 |
|---------|-------------------|
| 手动触发 /evolve | `cron` tool：定时触发每日 evolution run |
| — | `webhooks` tool：CI 跑完自动触发 evolution |
| — | `lobster` extension：将 evolution DAG 定义为声明式工作流 |
...
| 现在做法 | 生态里已有的替代/增强 |
|---------|-------------------|
| exec 读 stdout/stderr | `tmux` skill：连接到已有训练进程实时监控 |
| — | `log-dive` skill：跨多个 log 源的统一搜索 |
| — | `session-logs` skill：回溯历次 evolution run 的决策过程 |
...
Evo-anything 目前只使用了生态能力的**很小一部分**。整个生态里有：

- **353 个 Search & Research skills**，其中包含专门针对 arXiv / PubMed / 学术研究的工具，而 Evo-anything 用 browser 手动访问 paperswithcode
- **166 个 Git & GitHub skills**，而 Evo-anything 只用到了 `gh search repos`
- **392 个 DevOps & Cloud skills**，可以自动化 GPU 实例申请、benchmark 容器化、实验追踪
- **1,222 个 Coding Agents & IDEs skills**，可以将变体生成交给专业编程 agent（Claude Code / Codex）而不是 LLM 直接输出
- **`oracle` skill**：整仓库上下文分析，MapAgent 识别优化目标的能力可以大幅提升
- **`canvas` tool**：实时 fitness 曲线可视化，目前完全没用
- **`memory-lancedb` extension**：语义向量搜索历史 evolution，而不是只能线性读 Markdown

这份文档应当作为后续开发的参考：**优先集成生态里已有的能力，而不是重新造轮子**。
