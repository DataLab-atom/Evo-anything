...
> 目的：**搞清楚整个生态已经提供了什么**，哪些 OpenClaw-Evo 用到了，哪些还没用到

---

## 核心认知

OpenClaw-Evo 能做到"自己去搜文献、自己操作 git、自己运行代码、自己读代码分析日志"，**不是它自己实现的**。这些能力来自整个生态的基础设施，分三层：
...
**OpenClaw-Evo 重度依赖（MapAgent / WorkerAgent / ReflectAgent 全用）**
...
**OpenClaw-Evo 重度依赖（WorkerAgent 并行执行）**
...
**→ 可用于：将 OpenClaw-Evo 发现的最佳变体自动提 PR 到上游仓库**
...
**OpenClaw-Evo 的 MCP server 可通过此调试**
...
### A. Search & Research 品类（353个 skills）— OpenClaw-Evo 最相关
...
## 五、生态已有、OpenClaw-Evo 尚未使用的高价值能力
