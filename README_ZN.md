...
EvoClaw 是一个面向任意带有 benchmark 或评测脚本的 git 仓库的自动演化优化系统。它通过 LLM 驱动的**结构-功能协同演化**流程，自动识别可优化目标、生成代码变体、执行基准评测，并在多轮迭代中持续筛选和保留表现更优的实现。

作为一个集成到 OpenClaw/MCP 生态中的工程化演化引擎，EvoClaw 将 git 分支视为候选个体、将 benchmark 结果视为适应度，并结合多目标选择、策略约束和跨代记忆机制，对算法、模型训练代码及其他可量化评测的软件仓库进行自动、可追踪、可持续的优化。
...
LLM4AD 是我们的重要前序工作之一，它系统化地探索了如何利用大语言模型进行自动算法设计，并提供了面向算法任务的方法框架、评测流程和实验平台，为后续研究奠定了坚实基础。相比于 LLM4AD 主要面向研究平台和任务级接入，EvoClaw 更进一步，将这一能力扩展为面向任意 git 仓库的工程化自动演化系统。

EvoClaw 的核心优势在于更高程度的自动化与更自然的交互方式。对于以往通常需要人工编写或调整对接代码、手动组织实验流程的优化任务，EvoClaw 可以直接通过自然语言接收目标，自动完成仓库发现、环境准备、benchmark 对接、优化目标识别、代码变体生成、评测筛选和结果追踪，从而把“LLM 做算法/代码优化”从半手工研究流程推进到可持续运行的全自动闭环。
