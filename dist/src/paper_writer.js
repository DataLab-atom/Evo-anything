...
exports.generateIntroductionSection = generateIntroductionSection;
exports.generateRelatedWorkSection = generateRelatedWorkSection;
exports.generateExperimentsSection = generateExperimentsSection;
exports.generateMethodSection = generateMethodSection;
exports.generatePaperDraft = generatePaperDraft;
exports.generatePaperBundle = generatePaperBundle;
exports.resolveRepoOutputPath = resolveRepoOutputPath;
exports.writeGeneratedArtifact = writeGeneratedArtifact;
exports.writeGeneratedArtifacts = writeGeneratedArtifacts;
exports.validatePaperArtifacts = validatePaperArtifacts;
...
function renderAbstractText(text) {
...
function renderContributionParagraphs(contributions) {
...
function collectNodeTexts(forest, type, statuses) {
...
function defaultAbstract(forest) {
...
function defaultIntroduction(forest) {
    const questions = convergenceQuestions(forest);
    const primary = contributionDescriptions(forest, "primary");
    const paragraphs = [
    ...
    const contributionBlock = renderItemize(primary.length > 0
        ? primary
...
function generateIntroductionSection(forest, sectionTitle = "Introduction") {
    const questions = convergenceQuestions(forest);
    const primary = contributionDescriptions(forest, "primary");
    ...
    const latex = [
        ...
        renderItemize(primary.length > 0
            ? primary
...
function defaultRelatedWork(forest) {
...
function generateRelatedWorkSection(forest, sectionTitle = "Related Work") {
    const refs = collectReferenceKeys(forest);
...
function defaultExperiments(forest) {
...
function generateExperimentsSection(forest, sectionTitle = "Experiments") {
    const experimentIds = collectExperimentIds(forest);
    const evidenceTexts = collectNodeTexts(forest, "evidence", ["active", "converged"]);
    const primary = contributionDescriptions(forest, "primary");
    const questions = convergenceQuestions(forest);
...
function defaultConclusion(forest) {
    const primary = contributionDescriptions(forest, "primary");
    const questions = convergenceQuestions(forest);
...
function bibliographyTarget(outputPath, bibliographyPath) {
    const outputDir = outputPath ? (0, node_path_1.dirname)(outputPath) : "";
    const relativePath = outputDir ? (0, node_path_1.relative)(outputDir, bibliographyPath) : bibliographyPath;
    const noExt = (0, node_path_1.extname)(relativePath) ? relativePath.slice(0, -(0, node_path_1.extname)(relativePath).length) : relativePath;
    return noExt.replace(/\\/g, "/");
}
function inputTarget(mainOutputPath, sectionPath) {
    const relativePath = (0, node_path_1.relative)((0, node_path_1.dirname)(mainOutputPath), sectionPath);
    const noExt = (0, node_path_1.extname)(relativePath) ? relativePath.slice(0, -(0, node_path_1.extname)(relativePath).length) : relativePath;
    return noExt.replace(/\\/g, "/");
}
function sectionFilePath(sectionsDir, slug) {
    return (0, node_path_1.join)(sectionsDir, `${slug}.tex`);
}
...
function generateMethodSection(forest, sectionTitle = "Method") {
    const questions = convergenceQuestions(forest);
...
function generatePaperDraft(forest, options) {
...
function generatePaperBundle(forest, options) {
...
function resolveRepoOutputPath(repoPath, outputPath) {
    return (0, node_path_1.isAbsolute)(outputPath) ? outputPath : (0, node_path_1.join)(repoPath, outputPath);
}
function writeGeneratedArtifact(repoPath, outputPath, content) {
    const finalPath = resolveRepoOutputPath(repoPath, outputPath);
    (0, node_fs_1.mkdirSync)((0, node_path_1.dirname)(finalPath), { recursive: true });
    (0, node_fs_1.writeFileSync)(finalPath, content, "utf-8");
    return finalPath;
}
function writeGeneratedArtifacts(repoPath, files) {
    const written = {};
...
function validatePaperArtifacts(repoPath, mainOutputPath, bibliographyPath = "research/refs/references.bib") {
    const errors = [];
    const warnings = [];
    const checkedFiles = [];
    ...
    const mainContent = (0, node_fs_1.readFileSync)(mainPath, "utf-8");
    ...
    for (const target of inputMatches) {
        ...
        if (!(0, node_fs_1.existsSync)(sectionPath)) {
            errors.push(`Referenced section file not found: ${sectionPath}`);
            continue;
        }
        const sectionContent = (0, node_fs_1.readFileSync)(sectionPath, "utf-8");
    ...
    if (!(0, node_fs_1.existsSync)(bibPath)) {
